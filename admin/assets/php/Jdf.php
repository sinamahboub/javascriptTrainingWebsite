...
public function convertToJalali($dateGregorian,$separator="-",$num="en"){
	if (!isset($dateGregorian)) return null;
	if(strpos($dateGregorian,"-")>0)$array=explode("-",$dateGregorian);
	if(strpos($dateGregorian,"/")>0)$array=explode("/",$dateGregorian);
	return $this->tr_num($this->gregorian_to_jalali($array[0],$array[1],$array[2],$separator),$num);
}

public function convertToGregorian($dateJalali,$separator="-",$num="en"){
	if(!isset($dateJalali))return null;
	if(strpos($dateJalali,"-")>0)$array=explode("-",$dateJalali);
	if(strpos($dateJalali,"/")>0)$array=explode("/",$dateJalali);
